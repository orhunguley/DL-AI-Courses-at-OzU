	...
	public static void BreadthFirstSearch(Node root) {
		...
		while (!nodes.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (nodes.get(i).state[0] == 3 && nodes.get(i).state[1] == 1 && nodes.get(i).state[2] == 2) {
					...
					System.out.println("");
					System.out.println("Hired candidates are: " + nodes.get(i).totalCost);
					return;
				}
				
				if(nodes.get(i).children.size()!=0) {
				nodes.add(nodes.get(i).children.get(0));
				nodes.add(nodes.get(i).children.get(1));
				}
				nodes.remove(i);
			}
